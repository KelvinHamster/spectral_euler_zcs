...
def sim_KY_FF(L=300.0, a0 = 0.1, h0 = 1.0, X0 = 40, d0 = 0.1, s0 = 0.01,
        dx = 0.05, dt = 0.01, Xt = 30, mass_err_crit = 0.5,
        energy_err_crit = 0.5, tmax = 3000, v=0.7, M = 5, g = 9.81,
        tsave = 0, save_dir = None):
    """
    ...
    X0              [default: 40]
          - position (not vector index) of the soliton relative to offshore end 
...
#    sim_len = Nx * dx
 #   X0 += sim_len/2
    ...
    x,h = Simulator1D.KY_bathyFF(L,dx, h0, s0,d0,gamma, Ltoe)
    Nx = x.size
    init_cond = Simulator1D.solitonFF(X0,a0,h0,Nx,dx,g)
